...
void main() {
  ...
  group('buildStageSql', () {
    test('skips records already staged with same source and content hash', () {
      final sql =
          stage.buildStageSql([validRecord()], runSource: 'chedraui-mx');

      expect(sql, contains('where not exists'));
      expect(sql, contains('cross join records r'));
      expect(sql, contains('existing.source = r.payload->>\'source\''));
      expect(sql,
          contains('existing.content_hash = r.payload->>\'content_hash\''));
    });
  });
}
