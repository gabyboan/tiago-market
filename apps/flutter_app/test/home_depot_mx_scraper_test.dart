...
void main() {
  group('Home Depot branch-local scraping', () {
    test('parses store locator branches with geolocation', () {
      final branches = scraper.parseStoreLocatorBranches(jsonEncode({
        'PhysicalStore': [
          {
            'Description': [
    ...
    test('extracts branch price and inventory from search components', () {
      ...
      expect(products.single.productUrl,
    ...
    test('builds geolocated staging record from detail evidence', () {
      ...
      final detail = scraper.parseProductDetail(
        ...
        branch: branch,
        sourceUrl:
            'https://www.homedepot.com.mx/search/resources/api/v2/products?partNumber=111338',
      );
      ...
      expect(
          record['source_product_name'], contains('Pintura Berel Berelinte'));
